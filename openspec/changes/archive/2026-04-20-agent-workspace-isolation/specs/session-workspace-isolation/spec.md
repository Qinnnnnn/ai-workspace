...
All file-operating tools in the codenano SDK coreTools (FileRead, FileWrite, FileEdit, Bash, Glob, Grep) SHALL validate that any path accessed is within the session's workspace. Paths outside the workspace SHALL be rejected with an error.
...
Bash commands SHALL be validated to ensure all paths referenced in the command are within the workspace. Commands referencing paths outside the workspace SHALL be rejected.
