...
function TraceGroup({ message, animClass }: { message: UIMessage; animClass: string }) {
...
export function MessageBubble({ message }: MessageBubbleProps) {
  ...
  if (message.role === 'user') {
    return (
      <div className={cn('group ml-auto flex max-w-[min(85%,36rem)] items-center gap-2', baseAnim)}>
        <p
          className={cn(
      ...
      {empty && message.isStreaming ? (
        ...
        <>
          {message.traces && message.traces.length > 0 && (
            <TraceGroup message={message} animClass={baseAnim} />
          )}
